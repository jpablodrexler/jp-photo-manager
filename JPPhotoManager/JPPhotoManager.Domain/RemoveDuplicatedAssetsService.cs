...
namespace JPPhotoManager.Domain
{
    public class RemoveDuplicatedAssetsService : IRemoveDuplicatedAssetsService
    {
        ...
        {
            foreach (var duplicatedSet in duplicatedAssets) // TODO: IF MULTIPLE DUPLICATED SETS ARE PROCESSED, THIS THROWS AN ERROR OF MODIFIED COLLECTION
            {
        ...
        private void RemoveDuplicatesFromParentFolder(List<Asset> duplicatedSet)
        {
            ...
            if (duplicatedSet[0].Folder.IsParentOf(duplicatedSet[1].Folder))
            {
                ...
                this.moveAssetsService.DeleteAssets(new Asset[] { duplicatedSet[0] }, deleteFile: true);
            }

            // TODO: THIS CONDITION SHOULD BE ENCAPSULATED IN A STRATEGY PATTERN

            // If the second directory is parent of the first directory
            if (duplicatedSet[1].Folder.IsParentOf(duplicatedSet[0].Folder))
            {
                // TODO: TEMPORARY CODE, IMPLEMENT A LIST TO DISPLAY AS RESULT IN A DATAGRID.
                ...
                this.moveAssetsService.DeleteAssets(new Asset[] { duplicatedSet[1] }, deleteFile: true);
            }
        }
    }
}
