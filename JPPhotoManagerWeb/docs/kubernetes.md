...
- **A fourth catalog root, `/e2e-catalog`, *is* declared directly in `k8s/backend.yaml`** — unlike `catalog`/`catalog2`/`catalog3` above, it's an `emptyDir` (ephemeral, cluster-managed storage wiped whenever the Pod is recreated), not a `hostPath`, so it needs no real machine-specific path and is safe to version. It exists solely for the `release-e2e-suite` skill's generated test images (see that skill and `e2e-testing`) — real catalog data never belongs there, and nothing else should write to it.
