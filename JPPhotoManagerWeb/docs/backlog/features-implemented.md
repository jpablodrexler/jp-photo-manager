...
| #   | Change name                 | Priority | Schema Change | Effort | Area       | Brief description                                                                                                                                                                       | SDD Artifacts | Implementation |
| --- | --------------------------- | -------- | -------------- | ------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- |
...
`request-correlation-mdc` pairs with `global-error-handler` (#36, now implemented). The Angular `ErrorHandler` can read the `X-Request-ID` response header and include it in the error snackbar or log payload, linking a user-visible error directly to the backend log entries for that request. (Note: this block is duplicated from `features-planned.md`, where it remains because #43 is still pending.)
