...
Archive of features to the JPPhotoManagerWeb application that have been fully implemented. For pending features see `features-planned.md`.

The **Priority**, **Schema Change**, **Effort**, and **Area** columns were introduced after most of these features were already implemented, so existing rows show `—` (not tracked retroactively). Rows archived from `features-planned.md` going forward carry their real values across — see that file's column legend for the value vocabulary.

---

## Feature List

| #   | Change name                 | Priority | Schema Change | Effort | Area       | Brief description                                                                                                                                                                       | SDD Artifacts |
| --- | --------------------------- | -------- | -------------- | ------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
...
| 14  | `smart-albums`              | — | — | — | — | Extend albums with an optional `filter_json` column (same shape as saved search presets) so albums can be dynamically populated at query time; UI toggle between static and dynamic mode | ✅ Created |
| 18  | `timeline-view`             | — | — | — | — | Third gallery view mode grouping assets by year then month with sticky date headers; `GET /api/assets/timeline-groups` returns counts per bucket; buckets load lazily via Intersection Observer | ✅ Created |
| 21  | `video-file-support`        | — | — | — | — | Thumbnail generation via FFmpeg (`ProcessBuilder`) for `.mp4`/`.mov`/`.mkv`; catalog service accepts video MIME types; frontend shows play-overlay icon and `<video>` tag in the viewer  | ✅ Created |
| 34  | `openapi-documentation`     | — | — | — | — | Add `springdoc-openapi-starter-webmvc-ui` to `pom.xml`; annotate all `@RestController` classes with `@Operation` and `@ApiResponse`; exposes live Swagger UI at `/swagger-ui.html` and a machine-readable spec at `/v3/api-docs`; no schema change required; `/swagger-ui.html` is exempted from JWT authentication in `SecurityConfig` | ✅ Created |
| 39  | `api-rate-limiting`         | — | — | — | — | Add `bucket4j-spring-boot-starter` with per-IP token-bucket limits on `POST /api/auth/login` (brute-force prevention) and `GET /api/assets/catalog` (prevent accidental concurrent runs from multiple browser tabs); returns `429 Too Many Requests` with a `Retry-After` header when the bucket is exhausted; no schema change required | ✅ Created |
...
`server-side-spring-cache` (#28) was implemented first with `CaffeineCacheManager`. `redis-search-tag-cache` (#82) added the gallery pagination and tag-list endpoints to the caching scope established by #28 and upgraded the Spring cache manager from Caffeine to Redis; the existing `@Cacheable`/`@CacheEvict` annotations continued to work without modification when the `CacheManager` bean was switched to `RedisCacheManager`.
...
**Feature 72 — reverted by Feature 84**

`mongodb-exif-store` (#72)'s geospatial justification depended entirely on `gps-map-view` (#13), which was cancelled and removed from `docs/backlog/features-planned.md`. A codebase search confirmed the `2dsphere` index #72 introduced had never actually been queried by any feature — `$near`/`$geoWithin`/`geoNear` never appeared anywhere in the backend. `revert-exif-postgres-jsonb` (#84, `docs/backlog/features-planned.md`) has now restored `raw-exif-jsonb` (#63)'s PostgreSQL/JSONB design: `AssetExifRepositoryImpl` is JPA-backed again (`AssetExifEntity`, `JpaAssetExifRepository`, `AssetExifEntityMapper`), the `asset_exif` table was recreated by `V33__recreate_asset_exif.sql`, and the explicit `deleteByAssetId` cascade-delete calls were removed in favor of the restored `ON DELETE CASCADE` foreign key. Per explicit instruction, #84 did **not** migrate existing MongoDB `asset_exif` data back to PostgreSQL — the recreated table started empty, so any asset catalogued while #72 was in effect shows `null` EXIF data until its folder is re-catalogued. This row is kept (rather than deleted) to document *why MongoDB was tried and then undone* — useful history for anyone reconsidering geospatial EXIF storage in the future. MongoDB itself remains part of the stack, unaffected, for `mongodb-audit-log` (#73).
...
**Feature 84 — reverts Feature 72, restores Feature 63**

`revert-exif-postgres-jsonb` (#84) undid `mongodb-exif-store` (#72) and restored the `raw-exif-jsonb` (#63) PostgreSQL/JSONB design. #72's sole differentiating justification over #63 was a `2dsphere` geospatial index for `gps-map-view` (#13); #13 was cancelled and removed from `features-planned.md` before this change was proposed, and no other feature ever queried GPS proximity. `features-implemented.md`'s entry for #72 (above) is annotated as reverted rather than removed, preserving the historical record of why MongoDB was tried and why it was undone. MongoDB itself was unaffected — `mongodb-audit-log` (#73) keeps it in the stack regardless. No PostgreSQL↔MongoDB data migration was performed in either direction (explicit instruction); the recreated `asset_exif` table started empty.
...
**Feature 81 — overlap with Feature 28**

`redis-thumbnail-cache` (#81) is complementary to `server-side-spring-cache` (#28): #28 is per-JVM Caffeine caching for home stats, folder tree, and EXIF lookups; #81 adds a distributed Redis-backed cache for thumbnail bytes, an independent read path that #28 does not cover.

**Feature 82 — cache invalidation strategy**

`redis-search-tag-cache` (#82) upgraded the `CacheManager` from Caffeine to `RedisCacheManager`, adding `assets` and `tags` caches alongside the pre-existing `home-stats`, `sub-folders`, and `asset-exif` caches, all backed by the same `RedisConnectionFactory` used by the refresh-token store and thumbnail cache. `GetAssetsUseCaseImpl.execute(AssetFilter)` is annotated `@Cacheable(cacheNames = "assets", keyGenerator = "assetSearchCacheKeyGenerator")`, where `AssetSearchCacheKeyGenerator` builds `{folderId}:{sha256Hex(search|dateFrom|dateTo|minRating|sortCriteria|page|pageSize|includeDeleted|sortedTags)}` from the filter — the resulting Redis key is `assets:{folderId}:{hash}`. `AssetSearchCacheInvalidationListener` (`@KafkaListener` on `asset.cataloged` and `asset.deleted`, consumer group `asset-search-cache-invalidator`, following the same explicit-persistent-consumer-group pattern as `AuditLogKafkaListener`) evicts every `assets:{folderId}:*` entry for the affected folder using a cursor-based `RedisConnectionFactory.getConnection().scan(...)` (never a blocking `KEYS`); `asset.cataloged` events resolve `folderId` via `GetFolderIdByPathUseCase` first since the event only carries `folderPath`. `ListTagsUseCaseImpl.execute(String query)` caches under `tags` with the fixed key `all`, but only when `query` is null/blank (a non-blank query bypasses the cache entirely); `AddTagToAssetUseCaseImpl` and `RemoveTagFromAssetUseCaseImpl` each `@CacheEvict(cacheNames = "tags", key = "'all'")` on success. A `CacheErrorHandler` bean logs any Redis get/put/evict failure at `WARN` instead of propagating it, so all five caches fail open on a Redis outage rather than failing the request.

**Feature 82 — overlap with Feature 28**

`redis-search-tag-cache` (#82) is an extension of `server-side-spring-cache` (#28) rather than a parallel cache: it reused #28's `@Cacheable`/`@CacheEvict` annotation pattern and its `CacheManager` bean, only swapping the underlying implementation from `CaffeineCacheManager` to `RedisCacheManager` and adding two new cache names (`assets`, `tags`) to the five now managed by that single bean. Unlike `redis-thumbnail-cache` (#81), which added an independent Redis-backed cache alongside Caffeine, #82 migrated the entire `server-side-spring-cache` capability itself onto Redis.

**Features 32, 36, 38 — no dependencies**

`folder-watch-service`, `global-error-handler`, and `folder-stats-in-tree` have no hard dependencies on other pending features and can be delivered in any order. (Note: this block is duplicated from `features-planned.md`, where it remains because #32 and #38 are still pending.)

**Feature 43 → Feature 36**

`request-correlation-mdc` pairs with `global-error-handler` (#36, now implemented). The Angular `ErrorHandler` can read the `X-Request-ID` response header and include it in the error snackbar or log payload, linking a user-visible error directly to the backend log entries for that request.
