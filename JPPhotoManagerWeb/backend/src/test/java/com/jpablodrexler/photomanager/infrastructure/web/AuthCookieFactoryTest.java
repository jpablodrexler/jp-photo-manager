...
class AuthCookieFactoryTest {
    ...
    void refreshCookie_returnsHttpOnlyCookieScopedToAuthEndpoints() {
        ...
        assertThat(cookie.isHttpOnly()).isTrue();
        // Must cover /api/auth/sessions (GetActiveSessionsUseCaseImpl/RevokeSessionUseCase both
        // need this cookie to identify the current session), not just /api/auth/refresh.
