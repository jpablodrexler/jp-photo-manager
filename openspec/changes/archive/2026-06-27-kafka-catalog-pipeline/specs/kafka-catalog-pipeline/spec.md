...
Each application instance SHALL run a `@KafkaListener` (consumer group `sse-broadcaster`) subscribed to `job.catalog.progress`, `job.sync.progress`, and `job.convert.progress`. On receiving a message the listener SHALL look up the `SseEmitter` registered locally for that `runId` and write the event to it.
...
`application-test.yml` SHALL override `spring.kafka.bootstrap-servers` to the `@EmbeddedKafka` broker address so unit tests require no external Kafka.
