...
Specifies how users can move or copy selected assets to a different folder from the gallery. Covers the gallery actions menu, the Folder Picker Dialog, and the backend move/copy endpoint behaviour for both move (destructive) and copy (non-destructive) operations.
