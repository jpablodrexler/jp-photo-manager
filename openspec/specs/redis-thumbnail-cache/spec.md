# redis-thumbnail-cache

A Redis-backed L2 cache for thumbnail bytes sitting in front of the disk-backed `ThumbnailStorageServiceAdapter`, with a 24-hour TTL, `allkeys-lru` eviction, and cache invalidation on thumbnail deletion. No changes to the `ThumbnailPort` interface, its method signatures, or any of its callers — the cache is entirely internal to the adapter.
