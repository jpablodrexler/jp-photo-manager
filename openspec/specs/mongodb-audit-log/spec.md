...
Specifies the append-only MongoDB-backed audit trail of user actions (view, download, tag, rate, delete, sync-run, convert-run, catalog-run), populated both by a dedicated Kafka consumer for already-published events and by direct port calls for actions with no existing topic, and exposed through a paginated read API.
