...
Dual-write mirroring of refresh tokens into Redis (token hash, user-to-tokens set, id-to-token index) alongside the existing PostgreSQL persistence, as the first phase of migrating refresh-token storage off PostgreSQL. PostgreSQL remains the sole read source of truth during this phase; the `refresh-token` capability's observable behavior is unchanged.
