...
`revert-exif-postgres-jsonb` (#84) undid `mongodb-exif-store` (#72) and restored the `raw-exif-jsonb` (#63) PostgreSQL/JSONB design. #72's sole differentiating justification over #63 was a `2dsphere` geospatial index for `gps-map-view` (#13); #13 was cancelled and removed from `features.md` before this change was proposed, and no other feature ever queried GPS proximity. `features-implemented.md`'s entry for #72 (above) is annotated as reverted rather than removed, preserving the historical record of why MongoDB was tried and why it was undone. MongoDB itself was unaffected — `mongodb-audit-log` (#73) keeps it in the stack regardless. No PostgreSQL↔MongoDB data migration was performed in either direction (explicit instruction); the recreated `asset_exif` table started empty.
