...
Read `openspec/improvements.md` in full.

### 2. Collect pending improvements

From the `## Improvement List` table, collect every row whose **Implementation** column shows `⬜ Pending`. For each row record:
...
### 3. Determine which improvements are unblocked

An improvement is **blocked** if it has a hard dependency on another improvement that is still `⬜ Pending` in the Implementation column. Derive this from the **Hard implementation dependencies** sub-section of `## Dependencies`.

Build a blocked set: for each hard-dependency statement `A → B`, if B's Implementation is still `⬜ Pending`, mark A as blocked.

An improvement is **unblocked** if it is not in the blocked set.

### 4. Score and rank unblocked improvements
...
The `### Recommended implementation order` subsection lists improvements in suggested sequence. Add +10 for improvements that appear in that list (they have been explicitly ordered).

**Tier 4 — No schema change bonus**

Improvements noted in `## Dependencies` implementation notes as "no schema change" or "no Flyway migration" are simpler to deliver. Add +5 for these.

Select the highest-scoring unblocked improvement. Break ties by choosing the lower improvement number (smaller = older = likely simpler).
