...
Steps:
0. **Run the full release E2E regression suite before doing anything else in this action.** `release-e2e-suite` §0 always rebuilds and redeploys both images from the current branch itself before running a single spec (a mandatory precondition it enforces every invocation, not something this skill or the user needs to arrange beforehand) — so there is no "is it deployed yet" question to ask here; the suite deploys the `release/`/`hotfix/` branch under test as its own first step. Confirm with the user before invoking it (it redeploys the live cluster, then seeds and later tears down real test data against it, and the whole pass takes several minutes — see `release-e2e-suite`'s own scope/duration notes). Then:
   - **Confirmed**: invoke `release-e2e-suite` via the Skill tool and wait for it to complete.
     - **Suite passes**: continue to step 1.
     - **Suite fails**: surface the failing spec(s) to the user (per `release-e2e-suite` §3's screenshot/diagnosis guidance) and stop — do not draft the changelog, push, or open PRs for a release that just failed its own regression suite, unless the user explicitly overrides and asks to proceed anyway.
   - **User declines to run it at all**: proceed to step 1, but note in the final report that this release/hotfix shipped without the E2E gate (and, since the suite is what would have deployed it, without necessarily having been deployed to the live cluster at all).
...
- Never open the release/hotfix PRs on a failed `release-e2e-suite` run without the user explicitly overriding — 3c's step 0 is a real gate, not advisory. Don't reinterpret a failure as "probably fine" and continue silently.
- This skill never itself builds images or deploys to Kubernetes — 3c's step 0 only asks whether that's already been done and, if so, runs the verification suite. If the user wants a deploy performed, point them at `build-and-deploy-k8s.sh` or `feature-development` Phase 4 rather than attempting it here.
